...
def load_cache():
    try:
        with open('/root/cybercheck/cache.json', 'r') as file:
            return json.load(file)
    except (FileNotFoundError, json.JSONDecodeError):
        return {}

def save_cache():
    with open('/root/cybercheck/cache.json', 'w') as file:
...
script_name = "/root/cybercheck/cybercheck.py"
...
def show_menu():
    ...
    link = f'http://{ip}:{port}/token={user}' if port else "Defina a porta primeiro."
